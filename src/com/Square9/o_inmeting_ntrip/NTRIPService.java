...
public class NTRIPService extends Service {
    ...
    private final String DEBUGTAG = "NTRIPSERVICE";
    ...
    /*
    The system calls this method when the host activity wants to bind with the service, by calling bindService().
    ...
    {
        Log.d(DEBUGTAG, "NTRIP service bound to Activity: " + inMessenger.getBinder().toString());
        return inMessenger.getBinder();
    }

    // Inner Class to handle messages from clients
    public static class IncomingHandler extends Handler
    {

        private final WeakReference<NTRIPService> mTarget;

        public IncomingHandler(NTRIPService target)
        {
    ...
    private String GetOutputInfo(int infotype)
    {
        switch (infotype) {
            ...
            case 6: // Heading
                return new DecimalFormat("0.0").format(Heading) + "North";
            case 7: // H-DOP
                return "HDOP:" + HDOP;
            case 8: // V-DOP
                return "VDOP";
    ...
    public void onCreate() {
        super.onCreate();
        Log.d(DEBUGTAG, "Service onCreate() performing setup procedure");
    ...
    /*
     * The system calls this method when the host activity, requests that the service be started by calling startService()
     */
    @Override
    public int onStartCommand(Intent intent, int flags, int startId)
    {
        Log.d(DEBUGTAG, "Android OS called 'onStartCommand()' ");
        Log.d(DEBUGTAG, "Received start id " + startId + ": " + intent);
        return START_STICKY; // run until explicitly stopped.
    }

    private void LoadPreferences(Boolean NotifyOfChanges)
    {
        Log.d(DEBUGTAG, "Loading User Preferences");
        ...
        try {
            ...
            // Todo Mock locations should always be enabled --> not necessary anymore to disable it!
            // Remove the shared pref and hard code it ;-)
            boolean prefSendMockLocation = preferences.getBoolean("sendmocklocation", true);
            if (prefSendMockLocation != SendMockLocation)
            { //Changed from before
                ...
                {
                    Log.d(DEBUGTAG, "disabling MOCK location provider");
            ...
            String tSERVERIP = preferences.getString("ntripcasterip", "ntrip.flepos.be");
            Log.d(DEBUGTAG, "Loading preference NTRIP Caster IP: " + tSERVERIP);
            ...
            Log.d(DEBUGTAG, "Loading preference NTRIP Server Port: " + tSERVERPORT);
            ...
            String tUSERNAME = preferences.getString("ntripusername", "XIOS/Vandenryt");
            Log.d(DEBUGTAG, "Loading preference NTRIP User Name: " + tUSERNAME);
            ...
            String tPASSWORD = preferences.getString("ntrippassword", "gpstest");
            Log.d(DEBUGTAG, "Loading preference NTRIP Password: " + tPASSWORD);
            ...
            // Todo remove option: Manual location setting should be removed, testing purposes only
            if (tUseManualLocation.equals("manual"))
            {
                uUseManualLocation = true;
            }
    ...
    private String TheTimeIs()
    {
        ...
        return Make2Digits(hours) + ":" + Make2Digits(minutes) + ":" + Make2Digits(seconds) + " ";
    }
        ...
        public void run() {
            try
            {
                ...
                if (nsocket.isConnected())
                {
                    ...
                    //todo Should be the only option after prototype version
                    if (nProtocol.equals("ntripv1"))    // Critical section
                    {
                        // Build request message
                        // A client’s request is designed as an HTTP message
                        String requestmsg = "GET /" + nMountpoint + " HTTP/1.0\r\n";
                        requestmsg += "User-Agent: NTRIP RayLecoAndroidNTRIPClient/20131106\r\n";
                        requestmsg += "Accept: */*\r\n";
    ...
    private void ParseNetworkDataStream(byte[] buffer)
    {
        ...
        { // We're just starting up.
            ...
            if (NTRIPResponse.startsWith("ICY 200 OK")) // The caster accepts data after sending the message “ICY 200 OK”
            {// Data stream confirmed.
                ...
                Log.d(DEBUGTAG, "NTRIP: Connected to caster");
            }
            else if (NTRIPResponse.indexOf("401 Unauthorized") > 1)
            {
                ...
                Log.d(DEBUGTAG, "NTRIP: Bad username or password.");
                TerminateNTRIPThread(false);
            }
            ...
            else if (NTRIPResponse.length() > 1024)
        ...
        { // Data streaming mode. Forward data to bluetooth socket
    ...
    private Boolean ParseGPGGA(String line) {
        ...
        if (ary.length > 9) { // We have at least 9 fields
            ...
            if (inFixType > 0)
            {
                ...
                if (SendMockLocation)
                {
                    Log.d(DEBUGTAG, "PUSHING MOCK LOCATION:");
                    Log.d(DEBUGTAG, "Latitude: " + Double.toString(lat));
                    Log.d(DEBUGTAG, "Longitude: " + Double.toString(lon));
                    Log.d(DEBUGTAG, "Elevation: " + Float.toString(Elevation));
                    Log.d(DEBUGTAG, "Speed: " + Float.toString(Speed));
                    Log.d(DEBUGTAG, "Heading: " + Float.toString(Heading));
                    Log.d(DEBUGTAG, "Accuracy: " + Float.toString(FixAccuracy));
                    mock.pushLocation(lat, lon, Elevation, Speed, Heading, FixAccuracy);
                }
            }
        } else {
            FixType = 0;
    ...
    public void onDestroy()
    {
        ...
        if (SendMockLocation)
        {
            Log.d(DEBUGTAG, "Shutting down MOCK Location provider");
            SendMockLocation = false;
            mock.shutdown();
        }

        if (BTShouldAutoSwitch && BTwasDisabled)
        {
            Log.d(DEBUGTAG, "Disabling Bluetooth");
            // Turn BT off
